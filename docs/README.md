...
│   │   ├── main.py                    # 命令行回测接口
│   │   ├── jq_compat.py               # JQ兼容性模块
│   │   └── ...
│   ├── data/                          # 数据模块
│   │   ├── data.py                    # 数据处理模块
│   │   ├── data_akshare.py            # AKShare数据源
│   │   └── ...
│   ├── factor/                        # 因子模块
│   │   ├── factor.py                  # 因子检验核心程序
...
```bash
# 基本使用（不画图）
python -m src.factor.main_factor --start 2024-01-01 --end 2024-12-31 --factors VOL10

# 画图并弹窗显示
python -m src.factor.main_factor --start 2024-01-01 --end 2024-12-31 --factors VOL10 --plot true --plot-mode popup

# 画图并保存到文件
python -m src.factor.main_factor --start 2024-01-01 --end 2024-12-31 --factors VOL10 --plot true --plot-mode save --output-dir results/factor_test

# 查看帮助
python -m src.factor.main_factor --help
```
...
```bash
# 基本使用
python -m src.factor.factor

# 指定参数
python -m src.factor.factor --start 2024-01-01 --end 2024-12-31 --factors VOL10

# 查看帮助
python -m src.factor.factor --help
```

详细使用说明请参考：
- [docs/main_factor_usage.md](docs/main_factor_usage.md) - main_factor.py 使用文档
- [src/factor/README.md](src/factor/README.md) - 因子模块文档
...
新增支持特征预处理与分组子模型融合，命令行入口 `python -m src.ml.ml_pipeline`，核心步骤包括：
