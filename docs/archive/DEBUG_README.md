...
- **文件**: `src/factor/debug_talib_model.py`
- **功能**: 逐步执行完整的模型训练流程，包括环境设置、数据验证、QLib初始化、数据集创建、模型训练和结果保存

### 2. 核心模块调试

#### 数据处理
- **配置名称**: `src/factor/convert_sample.py: 数据转换`
- **功能**: 调试数据格式转换过程

#### 模型训练
- **配置名称**: `src/factor/workflow_main.py: 模型训练`
- **功能**: 调试核心模型训练流程

#### 回测评估
- **配置名称**: `src/factor/backtest_evaluation.py: 回测评估`
- **功能**: 调试回测和性能评估

#### 权重导出
- **配置名称**: `src/factor/export_scores.py: 权重导出`
- **功能**: 调试最终权重生成

#### TALIB因子生成
- **配置名称**: `src/factor/generate_talib_factors.py: 生成TALIB因子`
- **功能**: 调试TALIB因子计算
