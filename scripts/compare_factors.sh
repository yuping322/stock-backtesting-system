...
FILE2="results/factor_test_all_oss/README.md"  # 最近几年

OUTPUT_FILE="results/FACTOR_COMPARISON.md"
...
# 从文件2提取因子
grep "^|" "$FILE2" | grep -E "🟢 alive|🟡 warning|🔴 dead" | awk -F'|' '{print $2}' | tr -d ' ' | sort | uniq > "$TEMP2"
...
**文件2 (results/factor_test_all_oss/README.md)**:
...
- **文件1在文件2中的覆盖率**: $(awk "BEGIN {printf \"%.1f%%\", $COMMON_COUNT/$COUNT1*100}")
- **文件2在文件1中的覆盖率**: $(awk "BEGIN {printf \"%.1f%%\", $COMMON_COUNT/$COUNT2*100}")
...
- 文件2 (几年): [results/factor_test_all_oss/README.md](results/factor_test_all_oss/README.md)
