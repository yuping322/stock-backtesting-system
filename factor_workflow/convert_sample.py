...
DEFAULT_RAW_FILE = DATA_ROOT / "formatted_data.csv"
...
def load_raw(path: Path, factor_cols: Sequence[str]) -> pd.DataFrame:
    ...
    base_candidates: Iterable[str] = ["date", "stock", "market_cap", *PRICE_COLS, *INFO_COLS]
...
def merge_price_and_factors(df: pd.DataFrame, factor_cols: Sequence[str]) -> pd.DataFrame:
    ...
    combined[["open", "high", "low", "close"]] = combined[["open", "high", "low", "close"]].apply(
...
def main():
    ...
    merged_df = merge_price_and_factors(raw_df, FACTOR_COLUMNS)
    ...
    missing_after_merge = [c for c in FACTOR_COLUMNS if c not in labelled_df.columns]
    if missing_after_merge:
        raise ValueError(f"Missing factors after merge: {missing_after_merge}")

    feature_df, label_df, industry_series, mkt_cap_series = build_panel(labelled_df, FACTOR_COLUMNS)
    ic_df = compute_factor_ic(labelled_df, FACTOR_COLUMNS)
