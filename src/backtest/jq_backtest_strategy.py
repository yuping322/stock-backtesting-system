...
from src.backtest.backtest_engine import BacktestEngine, BaseStrategy, StrategyFactory
from src.backtest.config import SystemConfig, StrategyConfig
from src.data import data as data_module
import src.backtest.jq_compat as jq_compat  # Import our compatibility layer
    ...
    def prepare_stock_list(self, current_date: pd.Timestamp) -> List[str]:
        ...
        try:
            ...
            for code in initial_list[:100]:  # Limit to avoid too many API calls
                try:
                    val_df = data_module.get_valuation(code, date=current_date)
        ...
        try:
            ...
            for stock in stock_list[:30]:  # Limit API calls
                try:
                    ...
                    hist_data = data_module.load_oss_stocks(
                        codes=[stock],
