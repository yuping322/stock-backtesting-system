...
def get_all_factors_and_stocks(start_date: str, end_date: str) -> tuple[list[str], list[str]]:
    ...
    from factor_old.export_data import data
...
def run_workflow_pipeline():
    ...
    # 检查必要的数据文件是否存在
    required_files = [
        "../data/model_tasks/features_panel.pkl",
        "../data/model_tasks/label_panel.pkl",
        "../data/model_tasks/meta_series.pkl"
    ]
...
def create_unified_workflow_data(all_good_factors: Dict[str, List[str]]) -> bool:
    ...
    # 使用最近3个月的数据范围
    from factor_old.export_data import get_last_3_months
    ...
    from factor_old.export_data import export_formatted_csv

    csv_file = export_formatted_csv(
        codes=stocks,
        start_date=start_date,
        end_date=end_date,
        output_dir='../data/model_tasks',
        factors=factors,
...
def main():
    ...
    print("- ../data/model_tasks/formatted_data.csv: 宽表格式数据")
    print("- ../data/model_tasks/features_panel.pkl: 因子数据")
    print("- ../data/model_tasks/label_panel.pkl: 标签数据")
    print("- ../data/model_tasks/meta_series.pkl: 元数据")
