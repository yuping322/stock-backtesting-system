...
生成指定日期范围和股票池的因子数据，支持多个来源：
1. Builtin 因子
2. Qlib Alpha158/360 因子
3. OSS 因子 (jq_ 前缀)
4. TA-Lib 因子 (TALIB_ 前缀)

输出合并的因子文件，可用于下游分析或回测。
"""
...
def get_stock_codes(index_name: str = 'small', limit: Optional[int] = None) -> List[str]:
    """获取股票代码列表"""
    if not get_index_stocks:
        raise ValueError("get_index_stocks 函数不可用")

    codes = get_index_stocks(index_name)
    if limit:
        codes = codes[:limit]
    return codes


def generate_builtin_factors(stock_codes: List[str], start_date: str, end_date: str,
                           output_dir: str, factor_names: Optional[List[str]] = None):
    """生成内置因子"""
...
def generate_qlib_factors(stock_codes: List[str], start_date: str, end_date: str,
                        output_dir: str, variants: List[str] = None):
    """生成 Qlib 因子"""
    if variants is None:
        variants = ['158', '360', '158vwap', '360vwap']

    results = []
    for variant in variants:
        if variant == '158':
            result = qlib.generate_qlib_158_factors(stock_codes, start_date, end_date, output_dir)
        elif variant == '360':
            result = qlib.generate_qlib_360_factors(stock_codes, start_date, end_date, output_dir)
        elif variant == '158vwap':
            result = qlib.generate_qlib_158vwap_factors(stock_codes, start_date, end_date, output_dir)
        elif variant == '360vwap':
            result = qlib.generate_qlib_360vwap_factors(stock_codes, start_date, end_date, output_dir)
        else:
            continue
...
def generate_oss_factors(stock_codes: List[str], start_date: str, end_date: str,
                       output_dir: str, factor_names: Optional[List[str]] = None):
    """生成 OSS 因子"""
    if factor_names is None:
        config_file = Path('./config/available_factors.txt')
        if config_file.exists():
            with config_file.open() as f:
                factor_names = [line.strip() for line in f if line.strip()]

    if not factor_names:
        raise ValueError("未指定 OSS 因子名称")
...
def generate_talib_factors(stock_codes: List[str], start_date: str, end_date: str,
                         output_dir: str, factor_names: Optional[List[str]] = None):
    """生成 TA-Lib 因子"""
    if factor_names is None:
        from src.factor.generator.talib import TalibFactorListGenerator
        factor_names = TalibFactorListGenerator.generate_talib_factors()
...
def merge_all_factors(generated_files: List[str], output_file: str):
    """合并所有因子文件"""
    merge_factor_files(generated_files, output_file=output_file, how='outer')


def main():
    """主程序：生成因子数据"""
    parser = argparse.ArgumentParser(description='因子生成器综合程序')
    parser.add_argument('--generators', nargs='+', choices=['builtin', 'qlib', 'oss', 'talib'],
                       default=['builtin', 'qlib', 'oss', 'talib'],
                       help='要运行的因子生成器')
    parser.add_argument('--qlib-variants', nargs='+',
                       choices=['158', '360', '158vwap', '360vwap'],
                       default=['158', '360', '158vwap', '360vwap'],
                       help='Qlib 因子变体')
    parser.add_argument('--index', default='small',
                       help='股票池指数名称')
    ...
    parser.add_argument('--output-dir', default='./data/factor_tasks',
                       help='输出目录')
    ...
    # 获取股票代码
    try:
        stock_codes = get_stock_codes(args.index, args.limit)
    ...
    # 生成各个因子
    if 'builtin' in args.generators:
        print("生成内置因子...")
        try:
            result = generate_builtin_factors(stock_codes, args.start_date, args.end_date, str(task_dir))
            generated_files.append(result['factor_file'])
            print(f"✓ 内置因子生成完成: {result['factor_file']}")
        except Exception as e:
            print(f"✗ 内置因子生成失败: {e}")

    if 'qlib' in args.generators:
        print("生成 Qlib 因子...")
        try:
            results = generate_qlib_factors(stock_codes, args.start_date, args.end_date, str(task_dir), args.qlib_variants)
            for result in results:
                generated_files.append(result['factor_file'])
            print(f"✓ Qlib 因子生成完成: {len(results)} 个变体")
        except Exception as e:
            print(f"✗ Qlib 因子生成失败: {e}")

    if 'oss' in args.generators:
        print("生成 OSS 因子...")
        try:
            result = generate_oss_factors(stock_codes, args.start_date, args.end_date, str(task_dir))
            generated_files.append(result['factor_file'])
            print(f"✓ OSS 因子生成完成: {result['factor_file']}")
        except Exception as e:
            print(f"✗ OSS 因子生成失败: {e}")

    if 'talib' in args.generators:
        print("生成 TA-Lib 因子...")
        try:
            result = generate_talib_factors(stock_codes, args.start_date, args.end_date, str(task_dir))
            generated_files.append(result['factor_file'])
            print(f"✓ TA-Lib 因子生成完成: {result['factor_file']}")
        except Exception as e:
            print(f"✗ TA-Lib 因子生成失败: {e}")

    # 合并文件
    if args.merge and generated_files:
        print("合并因子文件...")
        merge_output = task_dir / f"factors_merged_{timestamp}.csv"
        try:
            merge_all_factors(generated_files, str(merge_output))
...
def copy_factor_to_directory(result_dict, target_dir: Path, tag: str) -> Path:
    """
    将因子文件复制到指定目录，便于统一合并
    """
    normalized = normalize_factor_result(result_dict)
    factor_file = normalized.get('factor_file')
    if not factor_file or not os.path.exists(factor_file):
        raise FileNotFoundError(f"无法找到因子文件: {factor_file}")
    target_dir.mkdir(parents=True, exist_ok=True)
    target_path = target_dir / f"{tag}.csv"
    shutil.copy(factor_file, target_path)
    return target_path
...
def test_oss_factors():
    ...
    try:
        ...
        demo_factors = available_factors
...
@pytest.mark.slow
def test_full_small_generators_latest_merge():
    """
    使用 Small 指数全量股票、最近 90 天数据，运行四个因子来源并合并输出
    """
    if not get_index_stocks:
        pytest.skip("get_index_stocks 未提供，无法获取 small 股票池")

    try:
        small_codes = get_index_stocks('small')
    except Exception as exc:
        pytest.skip(f"无法获取 small 股票池: {exc}")

    if not small_codes:
        pytest.skip("small 股票池为空")

    latest_end = datetime.today().date()
    latest_start = latest_end - timedelta(days=90)
    start_date = latest_start.strftime('%Y-%m-%d')
    end_date = latest_end.strftime('%Y-%m-%d')

    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    target_dir = Path('./data/factor_tasks') / f"task_full_small_{timestamp}"
    target_dir.mkdir(parents=True, exist_ok=True)

    generated_files = []

    builtin_result = builtin.generate_builtin_factors(
        stock_codes=small_codes,
        start_date=start_date,
        end_date=end_date,
        factor_names=None,
    ...
    generated_files.append(copy_factor_to_directory(builtin_result, target_dir, f"builtin_all_{timestamp}"))

    qlib_variants = [
        ('qlib_158', qlib.generate_qlib_158_factors),
        ('qlib_360', qlib.generate_qlib_360_factors),
        ('qlib_158vwap', qlib.generate_qlib_158vwap_factors),
        ('qlib_360vwap', qlib.generate_qlib_360vwap_factors),
    ]
    for variant, func in qlib_variants:
        qlib_result = func(
            stock_codes=small_codes,
            start_date=start_date,
            end_date=end_date,
        ...
        generated_files.append(copy_factor_to_directory(qlib_result, target_dir, f"{variant}_all_{timestamp}"))

    config_file = Path('./config/available_factors.txt')
    if not config_file.exists():
        pytest.skip(f"配置文件缺失: {config_file}")
    with config_file.open() as fp:
        oss_factors = [line.strip() for line in fp if line.strip()]
    if not oss_factors:
        pytest.skip("available_factors.txt 中无因子")
    oss_result = oss.generate_oss_factors(
        factor_names=oss_factors,
        stock_codes=small_codes,
        start_date=start_date,
        end_date=end_date,
        output_dir=str(target_dir)
    )
    generated_files.append(copy_factor_to_directory(oss_result, target_dir, f"oss_all_{timestamp}"))

    from src.factor.generator.talib import TalibFactorListGenerator

    talib_factors = TalibFactorListGenerator.generate_talib_factors()
    if not talib_factors:
        pytest.skip("TA-Lib 因子列表为空")
    talib_result = talib.generate_talib_factors(
        stock_codes=small_codes,
        start_date=start_date,
        end_date=end_date,
        factor_names=talib_factors,
        output_dir=str(target_dir)
    )
    generated_files.append(copy_factor_to_directory(talib_result, target_dir, f"talib_all_{timestamp}"))

    merge_output = target_dir / f"factors_full_small_merged_{timestamp}.csv"
    merge_factor_files(
        [str(path) for path in generated_files],
        output_file=str(merge_output),
        how='outer'
    )
    assert merge_output.exists(), "合并文件未生成"
    merged_df = pd.read_csv(merge_output)
    assert not merged_df.empty, "合并文件为空"
    assert 'date' in merged_df.columns and 'stock_code' in merged_df.columns, "合并结果缺少关键列"
