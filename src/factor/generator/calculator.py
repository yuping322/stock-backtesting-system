...
from .custom import load_custom_factors
...
def _create_calculator_from_callable(func: Callable, data_loader=None) -> FactorCalculator:
    """Wrap callable into the appropriate FactorCalculator"""
    if not callable(func):
        raise ValueError("提供的自定义因子不是可调用对象")

    sig = inspect.signature(func)
    param_count = len(sig.parameters)

    if param_count == 1:
        return OHLCVFactorCalculator(func, data_loader)
    elif param_count == 3:
        return CustomFactorCalculator(func)
...
def create_factor_calculator(
    ...
    # 优先级3.5: 自定义因子模块 (src/factor/generator/custom)
    if factor_name:
        custom_mapping = load_custom_factors()
        custom_func = custom_mapping.get(factor_name)
        if custom_func:
            return _create_calculator_from_callable(custom_func, data_loader)
    
    # 优先级4: 使用自定义函数
    if factor_func:
        return _create_calculator_from_callable(factor_func, data_loader)
