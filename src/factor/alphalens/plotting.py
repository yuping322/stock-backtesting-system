...
def _render_table_figure(title, df):
    """Render a pandas DataFrame as a matplotlib table and return the figure."""
    # If there's no data, do not create a placeholder figure.
    # Return None so callers can skip saving/showing empty table images.
    if df.empty:
        return None

    display_df = df.copy()
    fig_height = max(3.5, 0.6 * len(display_df.index) + 1.5)
    fig_width = max(6.0, 1.2 * len(display_df.columns) + 2.5)
...
def plotting_context(context="notebook", font_scale=1.5, rc=None):
    ...
    rc_default = {"lines.linewidth": 1.5,
                  # prefer sans-serif and include common CJK fonts so Chinese titles render
...
def plot_quantile_statistics_table(factor_data, return_df=False):
    ...
    if return_df:
        ...
        # Only render and show the figure when there is data to display.
        fig = _render_table_figure("Quantiles Statistics", quantile_stats.round(3))
        if fig is not None:
            # 立即触发一次 show，使得外部拦截逻辑能够捕获该图
            plt.show()
